...
/** 会议室管理 表单 */
defineOptions({ name: 'MeetingRoomsForm' })
...
const formData = ref({
  id: undefined,
  name: '',
  position: MeetingPosition.Headquarter,
  roomNo: undefined,
  capacity: 1,
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await MeetingRoomsApi.getMeetingRooms(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as MeetingRoomsVO
    if (formType.value === 'create') {
      await MeetingRoomsApi.createMeetingRooms(data)
      message.success(t('common.createSuccess'))
    } else {
      await MeetingRoomsApi.updateMeetingRooms(data)
